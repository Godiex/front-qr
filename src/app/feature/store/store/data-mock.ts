
export const product1 = {
  "id": 1,
  "logoUrl": "https://i.ibb.co/mcDzV9g/LOGO-TOGO.jpg",
  "position": { lat: 10.968540, lng: -74.781319  },
  "photos": [
    "https://i.ibb.co/8YZsbWm/CAMISA-1.jpg",
    "https://i.ibb.co/z27qDFS/CAMISA-2.webp",
    "https://i.ibb.co/ns04jvt/CAMISA-3.jpg",
    "https://i.ibb.co/T1vv2bv/CAMISA-4.jpg",
    "https://i.ibb.co/BKL6tPg/CAMISA-5.jpg",
    "https://i.ibb.co/fpF9H5F/CAMISA-6.jpg",
    "https://i.ibb.co/vwyDsKC/CAMISA-7.jpg"
  ],
  "productsMayAlsoLike": [],
  "customContent": [
    {
      "languageCode": "es",
      "content": {
        "productTitle": "Camisas",
        "authenticText": "Esta es una camisa auténtica. \nCódigo de autenticación: \nGWB183_1294_0X001\n",
        "productDetailsItemTitle": "Detalles del Producto",
        "youMayAlsoLikeTitle": "Productos que quizas buscas",
        "productDetails": "Descripción: Camisa de manga larga con y sin estampados. Disponible en varios colores y tallas. Ideal para ocasiones informales. \n\nColores Disponibles: Azul, Rojo, Verde, Gris\n\nTallas: S, M, L, XL,XXL",
        "materialsItemTitle": "Materiales",
        "materials": "Material: 100% algodón.\nComposición: Tejido suave y transpirable.\nDurabilidad: Resistente al desgaste.",
        "careTextTitle": "Cuidado y Mantenimiento",
        "careText": "Cuidado y Mantenimiento\nComposición: Tejido suave y transpirable.\nDurabilidad: Resistente al desgaste.",
        "whereImFromItemTitle": "Origen",
        "whereImFromText": "Descubre de dónde viene nuestra chaqueta y su historia.",
        "environmentImpactItemTitle": "Impacto Ambiental",
        "environmentImpactText": "Los materiales de este producto muestran lo siguiente Reducciones del impacto en comparación con lo habitual. versiones de los mismos materiales",
        "circularityItemTitle": "Circularidad",
        "circularityText": "Cuando esta prenda llega al final de su vida útil no tiene que ir a un vertedero. Hay muchas maneras para alargar la vida de la prenda o transformarla en algo nuevo. Deseche de forma segura en la basura local sistema de gestión. Este artículo no liberará ninguna sustancia nociva. al medio ambiente y no liberará gases de efecto invernadero. gases.",
        "complianceItemTitle": "Cumplimiento",
        "complianceText": "CHAMBRAY ORGÁNICO HECHO CON 70% ORGÁNICO ALGODÓN Y 30% ALGODÓN PROCEDENTE DE FUENTES RECICLADAS.\n\n• Producido a partir de algodón orgánico estadounidense que es sin productos químicos tóxicos ni OGM (genéticamente organismos modificados) \n\n• Teñido con pigmentos especiales libres de peligrosos. quimicos",
        "letsGetSocial": "Redes Sociales"
      }
    },
    {
      "languageCode": "en",
      "content": {
        "productTitle": "Shirts",
        "authenticText": "This is an authentic jacket. Authentication code: GWB183_1294_0X001",
        "productDetailsItemTitle": "Product Details",
        "youMayAlsoLikeTitle": "You also may like",
        "productDetails": "Description: Long-sleeve shirt with and without prints. Available in various colors and sizes. Ideal for casual occasions.\n\nAvailable Colors: Blue, Red, Green, Gray\n\nSizes: S, M, L, XL, XXL",
        "materialsItemTitle": "Materials",
        "materials": "Material: 100% cotton.\nComposition: Soft and breathable fabric.\nDurability: Wear-resistant.",
        "careTextTitle": "Care and Maintenance",
        "careText": "Machine wash in cold water. Do not use bleach. Dry at low temperature. Iron at medium heat. Do not dry clean. Do not wring.",
        "whereImFromItemTitle": "Origin",
        "whereImFromText": "Discover where our jacket comes from and its history.",
        "environmentImpactItemTitle": "Environmental Impact",
        "environmentImpactText": "This product’s materials show the following reductions in impact when compared to regular versions of the same materials",
        "circularityItemTitle": "Circularity",
        "circularityText": "When this garment reaches the end of its useful life it doesn’t have to go to a landfill. There are many ways to extend the life of the garment or transform it into something new. \nDispose of safely in your local waste management system. This item will not release any harmful substances into the environment and will release no greenhouse gases",
        "complianceItemTitle": "Compliance",
        "complianceText": "ORGANIC CHAMBRAY MADE WITH 70% ORGANIC COTTON AND 30% COTTON FROM RECYCLED SOURCES.\n\n• Produced from American organic cotton that's grown without toxic chemicals or GMOs (genetically modified organisms)\n\n• Dyed with special pigments free of hazardous chemicals",
        "letsGetSocial": "Lets get social"
      }
    },
    {
      "languageCode": "ja",
      "content": {
        "productTitle": "シャツ",
        "authenticText": "これは本物のジャケットです。認証コード：GWB183_1294_0X001",
        "productDetailsItemTitle": "製品の詳細",
        "youMayAlsoLikeTitle": "お探しの製品",
        "productDetails": "説明: 長袖のシャツ、プリントありとなし。さまざまな色とサイズで利用可能。カジュアルな場合に最適です。\n\n利用可能な色: 青、赤、緑、灰色\n\nサイズ: S、M、L、XL、XXL",
        "materialsItemTitle": "素材",
        "materials": "素材: 100%コットン。\n構成: 柔らかく通気性のある生地。\n耐久性: 摩耗に強い。",
        "careTextTitle": "お手入れとメンテナンス",
        "careText": "洗濯機で冷水で洗ってください。漂白剤は使用しないでください。低温で乾燥させてください。中温でアイロンをかけてください。ドライクリーニングはしないでください。絞らないでください。",
        "whereImFromItemTitle": "原産地",
        "whereImFromText": "ジャケットの原産地とその歴史をご紹介します。",
        "environmentImpactItemTitle": "環境への影響",
        "environmentImpactText": "この製品の材質は次のとおりです 通常と比較した場合の影響の軽減 同じ素材のバージョン",
        "circularityItemTitle": "循環型デザイン",
        "circularityText": "この衣服が耐用年数の終わりに達すると、埋め立て地に行く必要はありません。たくさんの方法があります衣服の寿命を延ばす、または衣服を次のように変えるため 新しい何か。地元 廃棄物として安全に廃棄してください マネジメントシステム。この商品は有害物質を放出しません 環境中に排出され、温室効果を放出しません ガス",
        "complianceItemTitle": "規制順守",
        "complianceText": "70％オーガニックで作られたオーガニックシャンブレー コットンおよびリサイクル資源からのコットン 30%。\n\n• アメリカのオーガニックコットンから作られています。有毒化学物質や遺伝子組み換え作物（遺伝子組み換え作物）を使用せずに栽培されています。改変生物）\n\n• 有害物質を含まない特殊な顔料で染色されています。化学薬品",
        "letsGetSocial": "ソーシャルネットワーク"
      }
    }
  ]
}

export const product2= {
  "id": 2,
  "logoUrl": "https://i.ibb.co/mcDzV9g/LOGO-TOGO.jpg",
  "position": { lat: 35.682839, lng: 139.759455  },
  "photos": [
    "https://i.ibb.co/r2yBwHq/JENAS1.jpg",
    "https://i.ibb.co/KGyFYG4/jeans-1.png",
    "https://i.ibb.co/dg71qhN/jeans-2.png",
    "https://i.ibb.co/F7WSDzj/JEANS-3.jpg",
    "https://i.ibb.co/fDxGd6F/JEANS-4.jpg",
    "https://i.ibb.co/dWcskHG/JEANS-5.jpg",
    "https://i.ibb.co/P69pNwC/JEANS-6.jpg",
    "https://i.ibb.co/myHHGqL/JEANS-7.jpg"
  ],
  "productsMayAlsoLike": [
    {
      "id": 3,
      "photoUrl": "https://i.ibb.co/DCKxxKB/CHAQUETA-1.jpg"
    },
    {
      "id": 4,
      "photoUrl": "https://i.ibb.co/x6QvCFs/DEPORTIVO-1.jpg"
    },
  ],
  "customContent": [
    {
      "languageCode": "es",
      "content": {
        "productTitle": "Jeans",
        "authenticText": "Estos son jeans auténticos. \nCódigo de autenticación: \nGWB183_1294_0X002\n",
        "productDetailsItemTitle": "Detalles del Producto",
        "youMayAlsoLikeTitle": "Productos que quizás buscas",
        "productDetails": "Descripción: Jeans clásicos de corte recto. Diseño moderno y versátil. \n\nColores Disponibles: Azul, Negro, Gris\n\nTallas: 28-40",
        "materialsItemTitle": "Materiales",
        "materials": "Material: 98% algodón, 2% elastano.\nComposición: Tejido resistente y elástico.\nDurabilidad: Costura reforzadas.",
        "careTextTitle": "Cuidado y Mantenimiento",
        "careText": "Cuidado y Mantenimiento\nComposición: Tejido resistente y elástico.\nDurabilidad: Diseño duradero.",
        "whereImFromItemTitle": "Origen",
        "whereImFromText": "Descubre de dónde vienen nuestros jeans y su historia.",
        "environmentImpactItemTitle": "Impacto Ambiental",
        "environmentImpactText": "Cuando esta prenda llega al final de su vida útil no tiene que ir a un vertedero. Hay muchas maneras para alargar la vida de la prenda o transformarla en algo nuevo. Deseche de forma segura en la basura local sistema de gestión. Este artículo no liberará ninguna sustancia nociva al medio ambiente y no liberará gases de efecto invernadero.",
        "circularityItemTitle": "Circularidad",
        "circularityText": "Cuando esta prenda llega al final de su vida útil no tiene que ir a un vertedero. Hay muchas maneras para alargar la vida de la prenda o transformarla en algo nuevo. Deseche de forma segura en la basura local sistema de gestión. Este artículo no liberará ninguna sustancia nociva al medio ambiente y no liberará gases de efecto invernadero.",
        "complianceItemTitle": "Cumplimiento",
        "complianceText": "Jeans fabricados con un 98% de algodón y un 2% de elastano. Cumplen con los estándares de calidad y sostenibilidad. No se utilizan productos químicos dañinos en su producción.",
        "letsGetSocial": "Redes sociales"
      }
    },
    {
      "languageCode": "en",
      "content": {
        "productTitle": "JEANS",
        "authenticText": "This is authentic jeans. \nAuthentication code: \nGWB183_1294_0X002\n",
        "productDetailsItemTitle": "Product Details",
        "youMayAlsoLikeTitle": "Products You Might Like",
        "productDetails": "Description: Classic straight-cut jeans. Modern and versatile design. \n\nAvailable Colors: Dark Blue, Black, Gray \n\nSizes: 28-40",
        "materialsItemTitle": "Materials",
        "materials": "Material: 98% cotton, 2% elastane. \nComposition: Resilient and stretchy fabric. \nDurability: Reinforced seams.",
        "careTextTitle": "Care and Maintenance",
        "careText": "Care and Maintenance \nMaterial: 98% cotton, 2% elastane. \nComposition: Resilient and stretchy fabric. \nDurability: Durable design.",
        "whereImFromItemTitle": "Origin",
        "whereImFromText": "Discover where our jeans come from and their history.",
        "environmentImpactItemTitle": "Environmental Impact",
        "environmentImpactText": "When this garment reaches the end of its useful life, it doesn't have to go to a landfill. There are many ways to extend the life of the garment or transform it into something new. Dispose of it safely in your local waste management system. This item will not release any harmful substances into the environment and will emit no greenhouse gases.",
        "circularityItemTitle": "Circularity",
        "circularityText": "When this garment reaches the end of its useful life, it doesn't have to go to a landfill. There are many ways to extend the life of the garment or transform it into something new. Dispose of it safely in your local waste management system. This item will not release any harmful substances into the environment and will emit no greenhouse gases.",
        "complianceItemTitle": "Compliance",
        "complianceText": "Jeans made with 98% cotton and 2% elastane. Compliant with quality and sustainability standards. No harmful chemicals are used in their production.",
        "letsGetSocial": "Social networks"
      }
    },
    {
      "languageCode": "ja",
      "content": {
        "productTitle": "ジーンズ",
        "authenticText": "これは本物のジーンズです。\n認証コード:\nGWB183_1294_0X004\n",
        "productDetailsItemTitle": "製品詳細",
        "youMayAlsoLikeTitle": "お探しの製品",
        "productDetails": "説明: クラシックなストレートカットのジーンズ。モダンで多目的なデザイン。\n\n利用可能な色: ブラック、ブルー、グレー\n\nサイズ: XS、S、M、L、XL",
        "materialsItemTitle": "素材",
        "materials": "素材: 綿 98%、エラスタン 2%。\n組成: 耐久性のある生地と暖かい裏地。\n耐久性: 長持ちするデザイン。",
        "careTextTitle": "お手入れとメンテナンス",
        "careText": "お手入れとメンテナンス\n冷水で機械洗いしてください。漂白剤を使用しないでください。低温で乾燥させてください。ドライクリーニングはしないでください。ブラシでこすらないでください。溶剤でシミを落とさないでください。浸漬しないでください。絞ったりしぼったりしないでください。",
        "whereImFromItemTitle": "起源",
        "whereImFromText": "ジーンズの起源と歴史をご紹介します。",
        "environmentImpactItemTitle": "環境への影響",
        "environmentImpactText": "この製品の材質は通常と比較して影響を軽減します。",
        "circularityItemTitle": "サーキュラリティ",
        "circularityText": "この衣服が耐用年数の終わりに達すると、埋め立て地に行く必要はありません。たくさんの方法があります衣服の寿命を延ばす、または衣服を次のように変えるため新しい何か。地元の廃棄物として安全に廃棄してくださいマネジメントシステム。この商品は有害物質を放出しません環境中に排出され、温室効果を放出しませんガス。",
        "complianceItemTitle": "コンプライアンス",
        "complianceText": "ジーンズは綿 98% とエラスタン 2% で作られています。品質と持続可能性の基準を満たしています。製造には有害な化学物質は使用されていません。",
        "letsGetSocial": "ソーシャルネットワーク"
      }
    }
  ]
}

export const product3= {
  "id": 3,
  "logoUrl": "https://i.ibb.co/mcDzV9g/LOGO-TOGO.jpg",
  "position": { lat: 39.8283, lng: 98.5795  },
  "photos": [
    "https://i.ibb.co/DCKxxKB/CHAQUETA-1.jpg",
    "https://i.ibb.co/ThW6H9r/CHAQUETA-2.jpg",
    "https://i.ibb.co/Qc0837K/CHAQUETA-3.jpg",
    "https://i.ibb.co/3rkWrz4/CHAQUETA-4.jpg",
    "https://i.ibb.co/xDDD9vW/CHAQUETA-5.jpg",
    "https://i.ibb.co/xDDD9vW/CHAQUETA-5.jpg",
    "https://i.ibb.co/wCGrM7Y/CHAQUETA-6.jpg",
    "https://i.ibb.co/x3WN0GN/chaqueta-1.webp",
    "https://i.ibb.co/zsH1Gzd/chaqueta-2.webp",
    "https://i.ibb.co/1TVSmwF/chaqueta-3.webp"
  ],
  "productsMayAlsoLike": [
    {
      "id": 2,
      "photoUrl": "https://i.ibb.co/r2yBwHq/JENAS1.jpg"
    },
    {
      "id": 4,
      "photoUrl": "https://i.ibb.co/x6QvCFs/DEPORTIVO-1.jpg"
    },
  ],
  "customContent": [
    {
      "languageCode": "es",
      "content": {
        "productTitle": "Chaqueta",
        "authenticText": "Esta es una chaqueta auténtica. \nCódigo de autenticación: \nGWB183_1294_0X008\n",
        "productDetailsItemTitle": "Detalles del Producto",
        "youMayAlsoLikeTitle": "Productos que quizás buscas",
        "productDetails": "Descripción: Chaqueta unicolor cuello alto acolchada con bolsillos en frente. Básico infaltable para combinar con todos tus looks en días fríos y de lluvia. \n\nColores Disponibles: Unicolor\n\nComposición: Exterior resistente al viento. Forro suave y transpirable.\n\nTallas: XS, S, M, L",
        "materialsItemTitle": "Materiales",
        "materials": "Material: 100% poliéster. Forro: 100% poliéster.\nComposición: Materiales de alta calidad y durabilidad.\nDurabilidad: Cremallera de alta calidad.",
        "careTextTitle": "Cuidado y Mantenimiento",
        "careText": "Cuidado y Mantenimiento\nLavar a máquina en agua fría. No usar blanqueador. Secar a baja temperatura. No lavar en seco. No frotar con cepillo. No remover manchas con solvente. No dejar en remojo. No retorcer ni exprimir.",
        "whereImFromItemTitle": "Origen",
        "whereImFromText": "Descubre de dónde viene nuestra chaqueta y su historia.",
        "environmentImpactItemTitle": "Impacto Ambiental",
        "environmentImpactText": "Los materiales de este producto muestran lo siguiente: Reducciones del impacto en comparación con lo habitual, versiones de los mismos materiales.",
        "circularityItemTitle": "Circularidad",
        "circularityText": "Cuando esta prenda llega al final de su vida útil, no tiene que ir a un vertedero. Hay muchas maneras para alargar la vida de la prenda o transformarla en algo nuevo. Deseche de forma segura en el sistema de gestión de basura local. Este artículo no liberará ninguna sustancia nociva al medio ambiente y no liberará gases de efecto invernadero.",
        "complianceItemTitle": "Cumplimiento",
        "complianceText": "Chambray orgánico hecho con un 70% de algodón orgánico y un 30% de algodón procedente de fuentes recicladas. Producido a partir de algodón orgánico estadounidense que es cultivado sin productos químicos tóxicos ni OGM (organismos genéticamente modificados). Teñido con pigmentos especiales libres de peligrosos químicos.",
        "letsGetSocial": "Redes Sociales"
      }
    },
    {
      "languageCode": "en",
      "content": {
        "productTitle": "Jackets",
        "authenticText": "This is an authentic jacket. \nAuthentication Code: \nGWB183_1294_0X009\n",
        "productDetailsItemTitle": "Product Details",
        "youMayAlsoLikeTitle": "Similar Products You May Like",
        "productDetails": "Description: Solid color high-neck quilted jacket with front pockets. A must-have essential to pair with all your outfits on cold and rainy days.\n\nMaterial: 100% polyester. Lining: 100% polyester.\n\nCare Instructions: Machine wash in cold water. Do not use bleach. Dry at a low temperature. Do not dry clean. Do not scrub with a brush. Do not remove stains with a solvent. Do not soak. Do not wring or squeeze.",
        "materialsItemTitle": "Materials",
        "materials": "Material: 100% polyester. Lining: 100% polyester.\nComposition: High-quality and durable materials.\nDurability: Reinforced stitching.",
        "careTextTitle": "Care and Maintenance",
        "careText": "Care and Maintenance\nMachine wash in cold water. Do not use bleach. Dry at low temperature. Do not dry clean. Do not scrub with a brush. Do not remove stains with a solvent. Do not soak. Do not wring or squeeze.",
        "whereImFromItemTitle": "Origin",
        "whereImFromText": "Discover where our jackets come from and their production history.",
        "environmentImpactItemTitle": "Environmental Impact",
        "environmentImpactText": "This product’s materials show the following reductions in impact when compared to regular versions of the same materials.",
        "circularityItemTitle": "Circularity",
        "circularityText": "When this garment reaches the end of its useful life, it doesn't have to go to a landfill. There are many ways to extend the life of the garment or transform it into something new. Dispose of safely in your local waste management system. This item will not release any harmful substances into the environment and will release no greenhouse gases.",
        "complianceItemTitle": "Compliance",
        "complianceText": "Organic chambray made with 70% organic cotton and 30% cotton from recycled sources. Produced from American organic cotton that's grown without toxic chemicals or GMOs (genetically modified organisms). Dyed with special pigments free of hazardous chemicals.",
        "letsGetSocial": "Social networks"
      }
    },
    {
      "languageCode": "ja",
      "content": {
        "productTitle": "ジャケット",
        "authenticText": "これは本物のジャケットです。 \n認証コード： \nGWB183_1294_0X010\n",
        "productDetailsItemTitle": "製品の詳細",
        "youMayAlsoLikeTitle": "お探しの製品",
        "productDetails": "説明：前面にポケットが付いたハイネックの単色のキルティングジャケット。寒い日や雨の日にあなたのすべてのルックに合わせるための不可欠なベーシックアイテム。\n\n素材：ポリエステル (ひゃくぱーせんと)。裏地：ポリエステル (ひゃくぱーせんと)。\n\nお手入れの方法：機械で冷水で洗ってください。漂白剤は使用しないでください。低温で乾燥させてください。ドライクリーニングはしないでください。ブラシでこすらないでください。溶剤でシミを取らないでください。浸け置きしないでください。絞らないでください。",
        "materialsItemTitle": "素材",
        "materials": "素材：ポリエステル (ひゃくぱーせんと)。裏地：ポリエステル (ひゃくぱーせんと)。\n\n構成：高品質で耐久性のある素材。\n\n耐久性：強化されたステッチ。",
        "careTextTitle": "お手入れとメンテナンス",
        "careText": "お手入れとメンテナンス\n機械で冷水で洗ってください。漂白剤は使用しないでください。低温で乾燥させてください。ドライクリーニングはしないでください。ブラシでこすらないでください。溶剤でシミを取らないでください。浸け置きしないでください。絞らないでください。",
        "whereImFromItemTitle": "原産地",
        "whereImFromText": "当社のジャケットの原産地と製造履歴をご覧ください。",
        "environmentImpactItemTitle": "環境への影響",
        "environmentImpactText": "この製品の材料は通常の材料と比較して以下の点で影響が軽減されています。",
        "circularityItemTitle": "循環性",
        "circularityText": "この衣服が寿命の終わりに達すると、埋め立て地に行く必要はありません。衣服の寿命を延ばすための多くの方法があり、新しい何かに変えることもできます。地元の廃棄物管理システムで安全に廃棄してください。この商品は環境に有害な物質を放出せず、温室効果ガスも放出しません。",
        "complianceItemTitle": "規制順守",
        "complianceText": "有機シャンブレーは70%が有機コットン、30%がリサイクルされた原料から作られています。アメリカの有機コットンから生産され、有害な化学物質や遺伝子組み換え作物を使用せずに栽培されます。特別な有害な化学物質を含まない顔料で染色されています。",
        "letsGetSocial": "ソーシャルネットワーク"
      }
    }
  ]
}
export const product4= {
  "id": 4,
  "logoUrl": "https://i.ibb.co/mcDzV9g/LOGO-TOGO.jpg",
  "position": { lat: 35.682839, lng: 139.759455  },
  "photos": [
    "https://i.ibb.co/KNrTnMC/deportiva-1.png",
    "https://i.ibb.co/k2nK6qQ/deportiva-2.png",
    "https://i.ibb.co/93Lx9Tg/deportiva-3.png",
    "https://i.ibb.co/x6QvCFs/DEPORTIVO-1.jpg",
    "https://i.ibb.co/1d28nc9/DEPORTIVO-2.jpg",
    "https://i.ibb.co/WKXf06z/DEPORTIVO-3.jpg",
    "https://i.ibb.co/gJxDXpn/DEPORTIVO-4.jpg",
    "https://i.ibb.co/tbHHG8S/DEPORTIVO-5.jpg"
  ],
  "productsMayAlsoLike": [
    {
      "id": 2,
      "photoUrl": "https://i.ibb.co/r2yBwHq/JENAS1.jpg"
    },
    {
      "id": 3,
      "photoUrl": "https://i.ibb.co/DCKxxKB/CHAQUETA-1.jpg"
    },
  ],
  "customContent": [
    {
      "languageCode": "es",
      "content": {
        "productTitle": "Ropa Deportiva",
        "authenticText": "Esta es ropa deportiva auténtica. \nCódigo de autenticación: \nGWB183_1294_0X005\n",
        "productDetailsItemTitle": "Detalles del Producto",
        "youMayAlsoLikeTitle": "Productos que quizás buscas",
        "productDetails": "Descripción: Conjunto de ropa deportiva para hombres y mujeres. Conjunto de yoga ideal para actividades al aire libre y entrenamiento en el gimnasio. \n\nColores Disponibles: Negro, Gris, Rojo\n\nComposición: Tejido elástico que facilita el movimiento.\n\nTallas: S, M, L, XL, XXL",
        "materialsItemTitle": "Materiales",
        "materials": "Material: 76% poliéster, 24% elastano. Tela Yoga: Nylon 90%, spandex 10%.\nComposición: Combinación de materiales resistentes y elásticos.\nDurabilidad: Costuras reforzadas.",
        "careTextTitle": "Cuidado y Mantenimiento",
        "careText": "Cuidado y Mantenimiento\nLavar a máquina en agua fría. No usar blanqueador. Secar a baja temperatura. No lavar en seco. No planchar sobre los logotipos. No dejar en remojo. No retorcer ni exprimir.",
        "whereImFromItemTitle": "Origen",
        "whereImFromText": "Descubre de dónde viene nuestra ropa deportiva y su historia.",
        "environmentImpactItemTitle": "Impacto Ambiental",
        "environmentImpactText": "Los materiales de este producto muestran lo siguiente: Reducciones del impacto en comparación con lo habitual, versiones de los mismos materiales.",
        "circularityItemTitle": "Circularidad",
        "circularityText": "Cuando esta prenda llega al final de su vida útil, no tiene que ir a un vertedero. Hay muchas maneras para alargar la vida de la prenda o transformarla en algo nuevo. Deseche de forma segura en el sistema de gestión de basura local. Este artículo no liberará ninguna sustancia nociva al medio ambiente y no liberará gases de efecto invernadero.",
        "complianceItemTitle": "Cumplimiento",
        "complianceText": "Chambray orgánico hecho con un 70% de algodón orgánico y un 30% de algodón procedente de fuentes recicladas. Producido a partir de algodón orgánico estadounidense que es cultivado sin productos químicos tóxicos ni OGM (organismos genéticamente modificados). Teñido con pigmentos especiales libres de peligrosos químicos.",
        "letsGetSocial": "Redes sociales"
      }
    },
    {
      "languageCode": "en",
      "content": {
        "productTitle": "Athletic Clothing",
        "authenticText": "This is authentic athletic clothing. \nAuthentication Code: \nGWB183_1294_0X006\n",
        "productDetailsItemTitle": "Product Details",
        "youMayAlsoLikeTitle": "Products You Might Like",
        "productDetails": "Description: Sportswear set for men and women. Ideal yoga set for outdoor activities and gym workouts. \n\nAvailable Colors: Black, Gray, Red\n\nComposition: Stretchy fabric for easy movement.\n\nSizes: S, M, L, XL, XXL",
        "materialsItemTitle": "Materials",
        "materials": "Sportswear Material: 76% polyester, 24% elastane. Yoga Fabric: 90% nylon, 10% spandex.\nComposition: A combination of durable and stretchy materials.\nDurability: Reinforced seams.",
        "careTextTitle": "Care and Maintenance",
        "careText": "Care and Maintenance\nMachine wash in cold water. Do not use bleach. Dry at low temperature. Do not dry clean. Do not iron over logos. Do not soak. Do not wring or squeeze.",
        "whereImFromItemTitle": "Origin",
        "whereImFromText": "Discover where our athletic clothing comes from and its history.",
        "environmentImpactItemTitle": "Environmental Impact",
        "environmentImpactText": "This product’s materials show the following reductions in impact when compared to regular versions of the same materials",
        "circularityItemTitle": "Circularity",
        "circularityText": "When this garment reaches the end of its useful life, it doesn’t have to go to a landfill. There are many ways to extend the life of the garment or transform it into something new. Dispose of safely in your local waste management system. This item will not release any harmful substances into the environment and will release no greenhouse gases.",
        "complianceItemTitle": "Compliance",
        "complianceText": "This athletic clothing is made with 76% polyester, 24% elastane, and 90% nylon, 10% spandex. It meets quality and sustainability standards. No harmful chemicals are used in its production.",
        "letsGetSocial": "Social networks"
      }
    },
    {
      "languageCode": "ja",
      "content": {
        "productTitle": "スポーツウェア",
        "authenticText": "これは本物のスポーツウェアです。\n認証コード:\nGWB183_1294_0X007\n",
        "productDetailsItemTitle": "製品詳細",
        "youMayAlsoLikeTitle": "お探しの製品",
        "productDetails": "説明: 男性と女性向けのスポーツウェアセット。アウトドア活動やジムでのトレーニングに最適なヨガセット。\n\n利用可能な色: 黒、グレー、赤\n\n構成: 動きを容易にする伸縮性のある生地。\n\nサイズ: S、M、L、XL、XXL",
        "materialsItemTitle": "素材",
        "materials": "スポーツウェア素材: ポリエステル 76%、エラスタン 24%。ヨガ用生地: ナイロン 90%、スパンデックス 10%。\n組成: 耐久性と伸縮性を兼ね備えた素材の組み合わせ。\n耐久性: 強化された縫い目。",
        "careTextTitle": "お手入れとメンテナンス",
        "careText": "お手入れとメンテナンス\n機械で冷水で洗ってください。漂白剤は使用しないでください。低温で乾燥させてください。ドライクリーニングはしないでください。ロゴ部分にアイロンをかけないでください。浸け置きしないでください。絞らないでください。",
        "whereImFromItemTitle": "起源",
        "whereImFromText": "スポーツウェアの起源と歴史をご紹介します。",
        "environmentImpactItemTitle": "環境への影響",
        "environmentImpactText": "この製品の材質は通常と比較して影響を軽減します。",
        "circularityItemTitle": "サーキュラリティ",
        "circularityText": "この衣服が耐用年数の終わりに達すると、埋め立て地に行く必要はありません。たくさんの方法があります衣服の寿命を延ばす、または衣服を次のように変えるため新しい何か。地元の廃棄物として安全に廃棄してくださいマネジメントシステム。この商品は有害物質を放出しません環境中に排出され、温室効果を放出しませんガス。",
        "complianceItemTitle": "コンプライアンス",
        "complianceText": "このスポーツウェアはポリエステル 76%、エラスタン 24%、ナイロン 90%、スパンデックス 10% で作られています。品質と持続可能性の基準を満たしています。製造には有害な化学物質は使用されていません。",
        "letsGetSocial": "ソーシャルネットワーク"
      }
    }
  ]
}

export const product10= {
  "id": 10,
  "logoUrl": "https://i.ibb.co/Wg0xJPD/Logo-ATLAS.jpg",
  "position": { lat: 10.986923160692335, lng: -74.80455866554756 },
  "photos": [
    "https://i.ibb.co/DQzcHVk/image1.jpg"
  ],
  "productsMayAlsoLike": [],
  "customContent": [
    {
      "languageCode": "es",
      "content": {
        "productTitle": "",
        "authenticText": "",
        "productDetailsItemTitle": "Informacion",
        "youMayAlsoLikeTitle": "Productos que quizás buscas",
        "productDetails": "<strong> Nombre completo: </strong> Carlos Andrés Pedraza López\n<strong> Identificación: </strong>1.234.685.792",
        "materialsItemTitle": "",
        "materials": "",
        "careTextTitle": "Arl - Eps",
        "careText": 'CUSTOM-CONTENTS-PDF',
        "whereImFromItemTitle": "Lugar asignado",
        "whereImFromText": "<strong> Conjunto las Delicias </strong>",
        "environmentImpactItemTitle": "",
        "environmentImpactText": "Salud total",
        "circularityItemTitle": "Informacion dotacion",
        "circularityText": "<strong> Última dotación entregada :</strong> 04/10/2023\n<strong> Dotaciones entregadas en el año:</strong> 2",
        "complianceItemTitle": "Calificación",
        "complianceText": "<div > <strong> Calificacion :</strong> <span style='font-size: 28px; color: gold;'>&#9733;</span><span style='font-size: 28px; color: gold;'>&#9733;</span><span style='font-size: 28px; color: gold;'>&#9733;</span><span style='font-size: 28px; color: gold;'>&#9733;</span> </div> <strong> Observacion: </strong>Tiene capacidad para solucionar inconvenientes bajo presión.",
        "letsGetSocial": "Redes sociales"
      }
    },
    {
      "languageCode": "en",
      "content": {
        "productTitle": "",
        "authenticText": "",
        "productDetailsItemTitle": "Information",
        "youMayAlsoLikeTitle": "Products You May Be Interested In",
        "productDetails": "Full Name: Carlos Andrés Pedraza López\nIdentification: 1,234,685,792",
        "materialsItemTitle": "",
        "materials": "",
        "careTextTitle": "Arl - Eps",
        "careText": 'CUSTOM-CONTENTS-PDF',
        "whereImFromItemTitle": "Assigned Location",
        "whereImFromText": "Las Delicias Community",
        "environmentImpactItemTitle": "",
        "environmentImpactText": "Health Total",
        "circularityItemTitle": "Uniform Information",
        "circularityText": "Last uniform issued: 10/04/2023\nUniforms issued this year: 2",
        "complianceItemTitle": "Rating",
        "complianceText": "Rating: ★ ★ ★ ★ ★ (5)\nNote: Capable of resolving issues under pressure.",
        "letsGetSocial": "Social Media"
      }
    }
  ]
}

export const reponseJson = [product1, product2, product3, product4, product10]
